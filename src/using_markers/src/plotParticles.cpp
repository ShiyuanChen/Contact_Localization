...
void ShapePlotter::updateTrueMarker()
{
    ...
    std::vector<double> tState;
    if(!n.getParam("true_state", tState)){
      ROS_INFO("Failed to get param: particle_frame");
      tState.resize(6);
    }

    //tf::Pose pose;
    part.pose.position.x = tState[0];
    part.pose.position.y = tState[1];
    part.pose.position.z = tState[2];
    part.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(tState[3],tState[4],tState[5]);
